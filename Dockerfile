...
ENV \
	...
	BUILD_VERSION="7.1-dev" \
	BUILD_ENV="dev" \
	BUILD_FROM="php:7.1-dev-fpm-alpine" \
	BUILD_PORTS_MAIN="9000" \
	BUILD_PORTS_ADDITIONAL="9001" \
	BUILD_PHP_VERSION="7.1-dev" \
...
ADD imports/bin/setup /usr/local/bin/setup/1518868517
ADD imports/bin/config /usr/local/bin/config/1518868517


RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1518868517 1>/dev/stdout 2>/dev/stderr
