...
ENV \
	...
	BUILD_VERSION="5.6-dev" \
	BUILD_ENV="dev" \
	BUILD_FROM="php:5.6-dev-fpm-alpine" \
	BUILD_PORTS_MAIN="9000" \
	BUILD_PORTS_ADDITIONAL="9001" \
	BUILD_PHP_VERSION="5.6-dev" \
...
ADD imports/bin/setup /usr/local/bin/setup/1518870759
ADD imports/bin/config /usr/local/bin/config/1518870759


RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1518870759 1>/dev/stdout 2>/dev/stderr
